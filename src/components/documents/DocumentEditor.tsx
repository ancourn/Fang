...
export function DocumentEditor({ document, workspaceId, channelId, onSave, onCancel }: DocumentEditorProps) {
  const { user } = useAuth();
  ...
  const handleVersionRestore = useCallback(async (versionNumber: number) => {
    // Refresh the document data after restore
    if (onSave) {
      // Trigger a save to refresh the data
      handleSave();
    }
  }, [handleSave, onSave]);
      ...
      <div className={`flex flex-col flex-1 ${showSidebar ? "pr-80" : ""}`}>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
            ...
            <TabsContent value="history" className="flex-1 p-0 m-0">
              <div className="flex-1 p-4">
                <DocumentVersionHistory
                  ...
                  currentVersion={1} // This would come from API
            ...
            <TabsContent value="comments" className="flex-1 p-0 m-0">
              <div className="flex-1 p-4">
                <DocumentComments
                  documentId={document?.id || ""}
                  canComment={true} // This would be based on permissions
                  onCommentAdded={() => {
                    // Refresh comments
                  }}
                />
        ...
        <div className="border-t p-4">
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            ...
            <div>
              Auto-save {lastSaved ? "enabled" : "disabled"}
            </div>
          </div>
        </div>
      </div>
